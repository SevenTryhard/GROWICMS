...
export async function POST(request: Request) {
  try {
    ...
    const telefonoExistente = await db.select().from(tablas.usuarios).where(eq(tablas.usuarios.telefono, telefono)).get();
    if (telefonoExistente) {
      return NextResponse.json({ error: "Este telefono ya esta registrado" }, { status: 409 });
    }
